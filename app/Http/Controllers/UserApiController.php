...
class UserApiController extends Controller
{
    ...
    public function change_password(Request $request){
        ...
        if(Hash::check($request->old_password, $User->password))
        {
            ...
            if($request->ajax() || $request->wantsJson()) {
                return response()->json(['error' => trans('api.user.incorrect_old_password')], 500);
            }else{
   ...
   //  dd($Providers);
    ...
    public function send_admin_request(Request $request) {
        $this->validate($request, [
            's_latitude' => 'required|numeric',
            'd_latitude' => 'required|numeric',
            's_longitude' => 'required|numeric',
            ...
            'payment_mode' => 'required|in:CASH,CARD,PAYPAL',
            'card_id' => ['required_if:payment_mode,CARD','exists:cards,card_id,user_id,'.Auth::user()->id],
            'rental_providers' => 'required|numeric',
        ]);
        ...
        if(count($Providers) < $request->rental_providers) {
            if ($request->ajax() || $request->wantsJson()) {
                return response()->json(['error' => trans('api.ride.no_engough_providers_found')], 422); 
            } else {
                return back()->with('flash_success', trans('api.ride.no_engough_providers_found'));
            }
        }
        ...
        try {
            ...
            $UserRequest->admin_id = Auth::user()->id;
            $UserRequest->current_provider_id = 0;
            ...
            $UserRequest->save();
            $parent_request = $UserRequest;

            for ($provider_key = 0; $provider_key < $request->rental_providers; $provider_key++) {
                $UserRequest = new UserRequests;
                $UserRequest->booking_id = Helper::generate_booking_id();
                $UserRequest->user_id = 0;
                $UserRequest->admin_id = Auth::user()->id;
                $UserRequest->parent_id = $parent_request->id;
                
                if((Setting::get('manual_request',0) == 0) && (Setting::get('broadcast_request',0) == 0)){
                    $UserRequest->current_provider_id = $Providers[$provider_key]->id;
                } else {
                    $UserRequest->current_provider_id = 0;
                }
                ...
                if($request->has('schedule_date') && $request->has('schedule_time')){
                 ...
                 if((Setting::get('manual_request', 0) == 0) && (Setting::get('broadcast_request',0) == 0)){
                    //Log::info('New Request id : '. $UserRequest->id .' Assigned to provider : '. $UserRequest->current_provider_id);
                    (new SendPushNotification)->IncomingAdminRequest($Providers[$provider_key]->id);
                }
                ...
                if(Setting::get('manual_request', 0) == 0){
                    foreach ($Providers as $key => $Provider) {
                        if ($key >= $provider_key) {
                            if(Setting::get('broadcast_request', 0) == 1){
                                (new SendPushNotification)->IncomingAdminRequest($Provider->id);
                            }
            ...
            if($request->ajax() || $request->wantsJson()) {
                ...
                return redirect('admin/adminrequest/' . $parent_request->id . "/edit");
            }
        } catch (Exception $e) {
            if($request->ajax() || $request->wantsJson()) {
                return response()->json(['error' => trans('api.something_went_wrong')], 500);
    ...
    public function admin_cancel_request(Request $request) {

        $this->validate($request, [
            'request_id' => 'required|numeric|exists:user_requests,id,admin_id,'.Auth::user()->id,
        ]);
        ...
        try{
            ...
            if(in_array($UserRequest->status, ['SEARCHING','STARTED','ARRIVED','SCHEDULED'])) {
                ...
                // Send Push Notification to User
                (new SendPushNotification)->AdminCancellRide($UserRequest);

                $this->parent_request_status_check($UserRequest->id);
    ...
    private function parent_request_status_check($id)
    {
        try {
            $CurrentUserRequest = UserRequests::findOrFail($id);

            $UserRequest = UserRequests::where('id', $CurrentUserRequest->parent_id)->first();

            $UserRequestChilds = UserRequests::AdminRequestChilds($UserRequest->id)
                ->get()->toArray();

            $AdminRequestCompleted = true;
            foreach ($UserRequestChilds as $key => $value) {
                if ($UserRequestChilds[$key]->status != "CANCELLED" && $UserRequestChilds[$key]->status != "COMPLETED") {
                    $AdminRequestCompleted = false;
                }
            }
            if ($AdminRequestCompleted) {
     ...
     * @return \Illuminate\Http\Response
     */
    public function admin_request_status_check($id) {
        try {
            $UserRequest = UserRequests::AdminRequest($id)
                                        ->get()->toArray();

            $UserRequestChilds = UserRequests::AdminRequestChilds($id)
                                        ->get()->toArray();

            $search_status = ['SEARCHING','SCHEDULED'];
            $UserRequestsFilter = UserRequests::AdminRequestAssignProvider($id, $search_status)->get(); 
            ...
            return response()->json(['data' => $UserRequest, 'child_data' => $UserRequestChilds, 'sos' => Setting::get('sos_number', '911'), 'cash' => (int)Setting::get('CASH', 1), 'card' => (int)Setting::get('CARD', 0),'currency'=>Setting::get('currency','$')]);
    ...
    public function admin_rate_provider(Request $request) {

        $this->validate($request, [
            'request_id' => 'required|integer|exists:user_requests,id,admin_id,'.Auth::user()->id,
            'rating' => 'required|integer|in:1,2,3,4,5',
            'comment' => 'max:255',
        ]);
...
//        }
        ...
        try{
            ...
            if($request->ajax() || $request->wantsJson()){
                ...
                return back()->with('flash_success', trans('api.ride.provider_rated'));
            }
        } catch (Exception $e) {
            if($request->ajax() || $request->wantsJson()){
                return response()->json(['error' => trans('api.something_went_wrong')], 500);
    ...
    public function estimated_fares(Request $request){
        ...
        $this->validate($request,[
            ...
            'd_longitude' => 'required|numeric',
            'service_type' => 'required|numeric|exists:service_types,id',
            'rental_providers' => 'required|numeric|min:0|not_in:0',
        ]);
    ...
    public function trip_counts(Request $request) {
        try{
            $UserRequests = UserRequests::UserTrips(Auth::user()->id)->get();
            return response()->json(['counts' => count($UserRequests)]);
        }
    ...
    public function test(Request $request)
    {
        ...
        if ($request->type == "user") {
            $push =  (new SendPushNotification)->sendPushToUser($request->id, trans('api.push.charged_from_wallet'));
        } else {
            $push = (new SendPushNotification)->sendPushToProvider($request->id, trans('api.push.charged_from_wallet'));
        }
