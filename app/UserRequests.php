...
class UserRequests extends Model
{
    ...
    /**
     * The admin who created the request.
     */
    ...
    {
        return $this->hasMany('App\UserRequests', 'parent_id', 'id');
    }
    ...
    {
        return $query->where('user_requests.user_id', '!=', 0)
    ...
    public function scopeAdminRequestHistory($query)
    {
        return $query->where('user_requests.admin_id', '!=', 0)
                    ->where('user_requests.parent_id', 0)
                    ->orderBy('user_requests.created_at', 'desc')
                    ->with('admin','childs','payment','provider');
    }

    public function scopeAdminRequest($query, $request_id)
    {
        return $query->where('user_requests.id', $request_id)
                    ->where('user_requests.admin_id','<>',0)
                    ->select('user_requests.*')
                    ->with('user','admin','provider','service_type','provider_service','rating','payment');
    }

    public function scopeAdminRequestChilds($query, $request_id)
    {
        return $query->where('user_requests.parent_id', $request_id)
                    ->where('user_requests.admin_id','<>',0)
                    ->select('user_requests.*')
                    ->with('user','admin','provider','service_type','provider_service','rating','payment');
    }

    public function scopeAdminRequestAssignProvider($query, $request_id, $check_status)
    {
        return $query->where('user_requests.parent_id', $request_id)
                    ->where('user_requests.provider_id',0)
                    ->whereIn('user_requests.status', $check_status)
